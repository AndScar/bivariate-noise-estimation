function [Absolute_Noise,Percentage_Noise,Apen,tgrid]=Raw_noise_evaluation_pp(X,dim,ris,range)
...
% semilogx(grid./std(X),Apen) or semilogx((ris:ris:1),Apen)



%if inputs are not enough, ris=0.001 and range=std(X) by default.
if(nargin<2)
...
%The expected noise level is upposed to be less than the 40% of the std(X). 
try
if (std(Apen)<0.01)
Absolute_Noise=0;%grid(min(find(abs(dif1(1:round(length(dif)*0.02)))==min(abs(dif1(1:round(length(dif)*0.02)))))));
