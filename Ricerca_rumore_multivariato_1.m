function [Absolute_Noise,Percentage_Noise,Apen,rgrid]=Ricerca_rumore_multivariato_1(X,dim,ris,range)
...
Y=-log(scala.^2)-Apen; %CASO MULTIVARIATO
...
Absolute_Noise=rgrid(max(find(max(Apen)==Apen))-1+min(find(abs(dif1(min(find(max(Apen)==Apen)):round(0.3*length(dif))))==min(abs(dif1(min(find(max(Apen)==Apen)):round(0.3*length(dif))))))));
Percentage_Noise=Absolute_Noise./range;
